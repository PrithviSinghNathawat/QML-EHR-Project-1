...
accuracy, and client parameter divergence vs heterogeneity (alpha).
...
skipped with a printed warning, not a crash -- Arm 4/5 results don't exist in
this checkout yet, and this must keep working once they land.
...
mean_pairwise_l2 (divergence).

Arm 1 (centralized) has no client axis and no alpha axis by construction --
it is not part of these three figures, which are all heterogeneity-vs-
something plots. That is a consequence of what Arm 1 is, not an oversight.
"""
import os
import warnings
...
def make_worst_client_figure(df: pd.DataFrame, color_map: dict, ylim=None):
    ...
    _plot_vs_alpha(
        ...
        "Worst-client accuracy vs. heterogeneity (primary result)",
