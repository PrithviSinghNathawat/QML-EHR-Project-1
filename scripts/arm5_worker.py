"""One Arm 5 (VQC + circular-mean aggregation) replicate: a single
(condition, seed, fold). Identical to scripts/arm4_worker.py except for
the aggregator (circular_mean instead of fedavg) and output directory --
the D-007 ablation citing A2G-QFL. Same resumability pattern.

Usage: arm5_worker.py <condition> <seed> <fold>
"""
import json
import os
import sys
...
from aggregators import circular_mean  # noqa: E402
...
def main():
    ...
    model, divergence = run_federated(
        lambda: VQCModel(seed=seed), circular_mean, client_data,
