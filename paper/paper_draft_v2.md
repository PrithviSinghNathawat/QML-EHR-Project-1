...
Worst-client accuracy declines monotonically with α for all three model families, consistent with prior reports of client-level disparity [3]. The middle three columns reproduce the original single-method decomposition; the right three columns are an independent training-effect estimate obtained by re-evaluating the same trained models against a pooled, α-invariant held-out set (no retraining, following the shared-test evaluation protocol used by [6]), with the worst-group statistic (a minimum over a fixed, α-independent 4-way split of that same held-out set) reported as the range's upper bound so the two methods are compared on matched statistics, not just matched data. **The "Decomposition composition (share)" column is not robust to a minimum-partition-size filter for LR and MLP (Section V-G). The "Implied composition under shared-test" column is more resistant for MLP and VQC, but not for LR — its 101–106% range is the low end of what this statistic takes under filtering (up to 160.3%), a direct consequence of LR's near-zero training effect combined with its already-small observed decline; see Section V-G before citing LR's specific percentage.**
...
**Consequence for the headline: qualification needed for heart disease LR specifically, not for the paper's overall claim.** The Abstract, Contributions, and Section V.A currently state LR's shared-test-implied composition share as a fixed range (101–106%, from the unfiltered t=0 estimate). That range is now shown to be the *low end* of values this statistic takes under a defensible robustness check, not a stable point — the true reportable statement for LR is closer to "composition accounts for at least the full observed decline, with the exact multiple sensitive to how small a client partition is allowed to count," not a specific percentage. This is Prithvi's call on how to reframe the Abstract/§V-A/Conclusion's LR-specific language — not resolved here, flagged plainly instead, per instruction not to adjust anything to preserve the original claim. **The paper's cross-model claim — that composition dominates the observed decline for all three families — is not threatened by this**, since MLP and VQC (dataset 1) and every dataset-2 configuration remain stable and all still show composition shares comfortably ≥67%; only the specific numeric range attached to LR's row needs revision.
