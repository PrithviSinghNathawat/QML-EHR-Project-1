...
3. **Confirmation of a pre-registered prediction that the confound worsens with client count**, tested by isolating client count as the only varying condition (same dataset, same α grid, same protocol, 4 vs. 130 simulated clients): the shared-test-implied composition share rises from 73–78% to 98–99% between the two client counts — the confound is largest in exactly the many-client regime most federated learning evaluations use.
...
|---|---|---|---|---|---|---|
| Logistic regression | 4.66 pp | 3.82 pp (82%) | 0.84 pp | −0.26 to −0.06 pp | 4.72–4.92 pp (101–106%) | 0.90–1.10 pp |
...
The question this section answers is not "does composition dominate training on a second dataset" (Section V.A already established that for the first) but **whether the two-estimator diagnostic itself — agreement as trust, disagreement as warning — holds up as a method when applied somewhere new.** We test this on a second, independently sourced clinical dataset (Diabetes 130-US Hospitals [17]; 71,518 first-encounters-per-patient after excluding repeat visits, 24 retained features, no per-record hospital identifier in the public release so no natural-partition arm is possible) for the two classical model families only, at both 4 and 130 simulated Dirichlet clients — the latter isolating client count as a variable on its own, holding the α grid and protocol fixed. **As with Section V.A, the "Decomposition composition (share)" column below is not robust to a minimum-partition-size filter for most of these rows (Section V-G, checked directly against this exact K=130/small-partition concern); the shared-test-derived columns are the ones this section's own conclusions rely on.**

| Dataset | Model | K | α pair | Observed | Decomposition composition (share) | Decomposition residual | Shared-test pooled (SE) | Interaction |
|---|---|---|---|---|---|---|---|---|
| Heart Disease (1) | LR | 4 | 100→0.1 | 4.66 pp | 3.82 pp (82.0%) | 0.84 pp | −0.26 to −0.06 pp | 0.90–1.10 pp |
...
| Diabetes 130 (2) | LR | 4 | 100→1.0 | 0.80 pp | 0.65 pp (80.6%) | 0.16 pp | 0.14 pp (SE 0.07) | −0.08 to 0.01 pp |
...
| Diabetes 130 (2) | MLP | 4 | 100→1.0 | 0.99 pp | 0.87 pp (88.2%) | 0.12 pp | 0.22 pp (SE 0.08) | −0.15 to −0.10 pp |
...
**The pre-registered client-count prediction is confirmed, on the reliable statistic.** Comparing the two client counts at the α values available to both (1.0 and 0.5), the decomposition's own share moves inconsistently — up at α=1.0, down at α=0.5, for both models — because the decomposition's own residual is unstable at these configurations (the negative-residual rows above). The **shared-test-implied** composition share (observed decline minus the shared-test estimate, as a fraction of observed decline — the statistic Section V.A already established as the more trustworthy of the two wherever the two estimators disagree) moves cleanly and substantially in every one of the four matched comparisons: for the MLP at α=1.0, **73–78% at K=4 to 98–99% at K=130**; at α=0.5, 87–91% to 96–98%. For LR, 71–82% to 93–94% (α=1.0) and 73–78% to 86–89% (α=0.5). **The confound is worst in exactly the many-client regime most federated learning studies actually use** — this dataset's 130-client condition is closer to typical cross-silo/cross-device federated learning study sizes than the first dataset's 4-client natural-site count was.

### G. Minimum-partition-size robustness check (P-019)
...
| Dataset | Model | K | Composition share, t=0 | t=5 | t=10 | t=15 | t=20 | Robust? |
|---|---|---|---|---|---|---|---|---|
| Heart Disease (1) | LR | 4 | 82.0% | 79.9% | 51.4% | 14.0% | **−174.6%** | **No** |
| Heart Disease (1) | MLP | 4 | 27.1% | 23.8% | 17.0% | 7.6% | 2.5% | **No** |
| Heart Disease (1) | VQC | 4 | 117.1% | 117.1% | 116.8% | 120.0% | 151.9% | Yes (through t=15) |
| Diabetes 130 (2) | LR | 4 | 130.4% | 135.9% | 136.0% | 135.6% | 135.6% | Yes |
| Diabetes 130 (2) | MLP | 4 | 87.2% | 83.3% | 79.0% | 73.9% | 73.9% | Partial |
| Diabetes 130 (2) | LR | 130 | 80.6% | 70.2% | 56.7% | 59.1% | 57.4% | **No** |
| Diabetes 130 (2) | MLP | 130 | 76.7% | 68.1% | 55.3% | 52.9% | 48.8% | **No** |

**Verdict, stated plainly per instruction: the raw decomposition's own composition-share number is not robust to a minimum-partition-size filter in 5 of 7 configurations.** Only the quantum classifier (dataset 1) and logistic regression at K=4 (dataset 2) hold their value within a few points across the full threshold sweep. The other five move substantially and, in every case, monotonically downward as the threshold rises — heart disease LR moves from 82.0% (t=0) to 14.0% (t=15) and then becomes numerically unstable, flipping sign to −174.6% at t=20, because both the observed decline (4.67pp → 0.43pp) and the composition-only decline (3.83pp → −0.75pp) shrink toward zero together as the noisiest tiny partitions are excluded, and a ratio of two near-zero numbers is not a stable statistic. Heart disease MLP moves from 27.1% to 2.5% over the same sweep — an order of magnitude, in the same direction the shared-test estimate has argued for all along (more training effect, less composition, as the tail's composition-only noise is filtered out). Dataset 2's K=130 rows move by 20–24 points before roughly leveling off past t=10.

**This does not overturn the paper's headline claim, and reinforces the case for the shared-test estimate rather than weakening it.** The Abstract, Contributions, and Section V.A's actual "composition dominates all three families" claim were built on the **shared-test-implied** composition share, not the raw decomposition share tested here — and the shared-test estimate is structurally immune to this exact instability, since its primary statistic (pooled accuracy) is a mean over the full held-out set, not a minimum over individually skewed client partitions. This check makes that choice's justification concrete rather than assumed: the raw decomposition share, which Section V.A already flagged as unreliable relative to the shared-test estimate for the MLP specifically, turns out to also be fragile to an unrelated nuisance parameter (minimum partition size) for most configurations — direct, independent evidence that its instability is not confined to the one case where a disagreement happened to already be visible. **Being robust to the threshold is not the same as being correct**, however: dataset 2's LR at K=4 holds a stable ~130–136% share across every threshold tested, yet stays far from the shared-test-implied share for the same configuration (76.5%, Section V.F) at every one of them — filtering the tail does not, by itself, reconcile the decomposition with the more trustworthy estimate; it only sometimes reduces the size of the disagreement (heart disease MLP, dataset 2 K=130) and sometimes leaves it essentially unchanged (dataset 2 LR at K=4).
