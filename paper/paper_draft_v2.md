...
FedAvg [1] established the standard protocol and identified non-IID client data as a source of degradation. FedProx [2] adds a proximal term to the local objective, penalising divergence from the global model received at the start of each round — the same mechanism our own client parameter-divergence metric (Section V-B, Figure 5) measures directly. NIID-Bench [6] provides the closest methodological precedent to our study design: a controlled comparison of federated algorithms across Dirichlet-partitioned non-IID settings, concluding that no algorithm dominates across conditions — verified directly against their own Table III (e.g. CIFAR-10 at Dirichlet(0.5): FedAvg 68.2% vs. FedProx 67.9%; rcv1: FedAvg 48.2% vs. FedProx 70.3%), so if our own Arm 3 does not clearly beat Arm 2 at low α, that matches this established result rather than indicating an implementation defect. It evaluates on a shared held-out set and is therefore structurally unaffected by the confound we describe.

### B. Client-level performance and fairness

Agnostic Federated Learning [4] established that the aggregate objective carries no per-client guarantee, proposing a minimax formulation over client mixtures. q-FFL [3] defines fairness as uniformity of the accuracy distribution across devices and reports, for the unweighted objective, that average accuracy falls approximately 6.6 percentage points across a heterogeneity sweep (89.2% → 82.6%) while worst-decile accuracy falls approximately 45 points (70.9% → 25.5%, their Appendix Table 10) — a precedent for the *disparity* between the two, not for the specific shape of our own result: their setup offers no convex-vs-non-convex comparison, and their device counts (50–100) are an order of magnitude above our 4 clients. Ditto [5] proposes a distinct response to the same disparity problem — rather than reweighting the server-side objective toward poorly-performing clients (q-FFL's approach), each client keeps a personalised model regularised toward the global one, pursuing fairness and robustness to adversarial clients simultaneously; we do not evaluate personalisation in this project (out of scope), but Ditto is evidence the client-disparity problem remains an active area with multiple competing proposed fixes, not a narrow or solved concern. Two more recent works extend this evidence base: FedGA [18] reports a persistent gap between mean and bottom-decile accuracy under FedAvg/FedProx baselines on a different benchmark, motivating a Gini-coefficient-based reweighting scheme; Naseer and Shoaib [19] report worst-client accuracy gaps of 50.1pp (large, pretrained model) vs. 32.2pp (small model) under extreme label skew on a text-classification task — evidence the disparity phenomenon generalises across model scale, not only across q-FFL's specific setting; checked directly against their own accuracy table, their *global* accuracy also varies substantially across their sweep (86.6–97.8% for the small model), so, as with q-FFL, we cite this work for the disparity it reports, not for a global-accuracy-flatness finding its own data does not show.
...
The distinction between local-test and global-test evaluation is recognised in personalised federated learning, where both are sometimes reported to separate personalisation from generalisation [15], and client bias in personalised FL evaluation has been noted [16] — pFL-Bench's explicit design ("train/val/test splitting is conducted within the local data of each client") does not raise the evaluation-composition confound as a concern. Client-local train/test splitting under matching skew is standard practice more broadly — confirmed directly at the source-code level for q-FFL [3] (`generate_synthetic.py` splits train/test *within* each device's own generated distribution) and Ditto [5] (`fedbase.py`'s `test()` evaluates each client against its own local test data). NIID-Bench [6] is the checked exception, by construction rather than by decomposition: verified directly from its `utils.py` that it evaluates every party against a single shared test set, never partitioned per party — the confound cannot arise there, the same mechanism behind its never reporting per-client accuracy at all. This literature addresses evaluation *at* a given heterogeneity level. We did not find work decomposing the confound that arises when heterogeneity is *swept* under client-local evaluation.

### D. Synthetic partitioning

Dirichlet-based label partitioning [7] is the standard mechanism for synthesising non-identical client distributions and is the source of our independent variable. That work reports substantial global accuracy degradation for convolutional networks (non-convex) on ten-class vision data — their most-skewed CIFAR-10 setting reaches a baseline accuracy around 30.1%, well below their more-IID settings — a contrast we return to in Section VI, since our convex model shows none under the same partitioning method.
...
We did not find prior work that decomposes observed client-level degradation into training and evaluation-composition components under a swept heterogeneity protocol, nor work calibrating a real multi-institutional clinical partition against the synthetic Dirichlet scale. We checked the closest candidate directly: a federated-EHR heterogeneity study across seven real hospitals (AKI and sepsis risk prediction in critical care) [20] uses only real institutional splits, with no synthetic Dirichlet comparison at all — evidence that a real-vs-synthetic calibration on clinical data is not already covered elsewhere, not proof that no such paper exists anywhere.
...
**Figure 1.** `results/figs/composition_decomposition.png` — observed decline, composition-only decline, and residual (training effect), in percentage points, for each of the three model families at α=100→0.1. The three-bar grouping makes the pattern in the table above visible at a glance: LR and VQC's residual bars are small (VQC's is negative, below the zero line); the MLP's residual bar is the tallest of the nine, dominating its own observed-decline bar.
...
**Figure 2** (`results/figs/worst_client_accuracy_vs_alpha.png`) **and Figure 3** (`results/figs/global_accuracy_vs_alpha.png`) **share a y-axis range** (0.0–0.8, computed jointly from both datasets — `scripts/plots.py::_shared_accuracy_ylim`), so the contrast between them is a direct visual comparison, not an artifact of independently-scaled axes: worst-client accuracy (Figure 2) declines visibly across all five arms as α falls from 100 to 0.1, while global accuracy (Figure 3) stays within a narrow band across the same sweep — the flat-global/declining-worst-client pattern that motivates this paper's whole framing, shown before any decomposition is applied. **Both figures' dashed "natural" reference lines now read "α∈[1.0,1.7], P-016"** — regenerated for this pass (`scripts/plots.py::_plot_vs_alpha`'s legend label, the single source of the stale "~α 0.5–1.0" text on every figure that used it, fixed once rather than patched per-figure) so the legend agrees with §V-D rather than contradicting it.

**The pattern holds across the sweep, not only at the extreme.** The table above reports only the α=100→0.1 headline pair. Repeating the same decomposition at the intermediate conditions (α=100→1.0, α=100→0.5) for all three model families:

| Model | α pair | Observed (pp) | Comp.-only (pp) | Residual (pp) | Decomp. share | Shared-test TE (pp) | Implied share |
|---|---|---|---|---|---|---|---|
| LR | 100→1.0 | 2.53 | 1.49 | 1.04 | 58.9% | −0.22 | 108.7% |
| LR | 100→0.5 | 3.43 | 3.29 | 0.14 | 95.9% | −0.15 | 104.4% |
| LR | 100→0.1 | 4.66 | 3.82 | 0.84 | — (below 2pp floor under the robustness sweep, §V-G) | −0.26 to −0.06 | — |
| MLP | 100→1.0 | 2.64 | 1.43 | 1.21 | 54.2% | 0.08 | 97.0% |
| MLP | 100→0.5 | 4.77 | 3.01 | 1.76 | 63.1% | 0.11 | 97.7% |
| MLP | 100→0.1 | 18.46 | 4.99 | 13.47 | 27.0% | 4.62 | 75.0% |
| VQC | 100→1.0 | 2.66 | 3.29 | −0.63 | 123.7% | −0.18 | 106.8% |
| VQC | 100→0.5 | 5.44 | 5.21 | 0.23 | 95.8% | −0.24 | 104.4% |
| VQC | 100→0.1 | 7.25 | 8.50 | −1.25 | 117.2% | −0.81 to 0.35 | 95.2% |

Every intermediate value is consistent with the same qualitative story reported at the headline condition: LR and VQC's residuals stay small at every α (implied share 104–109%, i.e. no training effect beyond noise, at both 1.0 and 0.5, matching the 0.1 finding); the MLP's residual grows monotonically with heterogeneity (1.21pp → 1.76pp → 13.47pp) and its implied composition share correspondingly shrinks (97.0% → 97.7% → 75.0%) — the training effect is not a step function that appears only at the most extreme condition tested, it grows continuously across the sweep. `results/composition_decomposition_summary.csv` and the `client=='global'` rows of `results/diagnostic_results.csv`/`results/arm4_diagnostic_results.csv` are the source data; arithmetic per `scripts/composition_controlled_eval.py::cce_paired_estimate`.

**Natural partition: real four-site institutional heterogeneity produces essentially no worst-client degradation for any model family.** Section V-D calibrates what synthetic α the natural partition's *label-distribution skew* resembles (α ∈ [1.0, 1.7]); this is the separate question of what the natural partition actually does to worst-client accuracy, decomposed the same way as every synthetic condition above:

| Model | Observed decline (pp) | Composition-only decline (pp) | Decomposition residual (pp) | Shared-test TE (pp) |
|---|---|---|---|---|
| LR | 0.10 | −0.54 | 0.64 | 0.38 |
| MLP | −0.86 | −0.04 | −0.82 | −0.03 |
| VQC | 0.74 | 1.08 | −0.34 | −0.60 |

All three observed declines fall below the 2pp reporting floor (§III-C), so no percentage is reported for any of them — reported here in absolute terms only, per that rule. **The finding itself does not need a percentage to state plainly: at this archive's real four-site partition, worst-client accuracy under the natural split is statistically indistinguishable from worst-client accuracy at α=100 (near-IID) for every model family tested, including the MLP** — the same MLP whose synthetic-heterogeneity residual grows to 13.47pp at α=0.1. One family (MLP) shows a *negative* observed decline under the natural partition (−0.86pp: natural-partition worst-client accuracy is very slightly *higher* than the α=100 baseline, well within noise), which is itself informative about how mild this real partition is relative to any synthetic condition tested. Source: same files as the sweep table above, `condition == "natural"` rows.

**Figure 4.** `results/figs/partition_natural_vs_dirichlet.png` — per-client class balance (stacked bars, class 0 vs. class 1 record counts) for the natural 4-site partition alongside Dirichlet draws at α=100, 1.0, 0.5, and 0.1. The natural panel's four bars are visibly close in both size and class mix; α=0.1's panel shows one client at ~510 records nearly all one class and another at ~40 records — the class of skew this section's near-zero degradation finding says does *not* occur in the real institutional split.
...
**Figure 5.** `results/figs/client_divergence_vs_alpha.png` — mean pairwise L2 distance between client parameters at the final round, vs. α, for every arm. All curves rise as α falls, LR (convex) rising the least and MLP/FedProx variants rising the most steeply — the mechanism FedProx directly targets, shown separately from its effect on accuracy (§V-B above) so the two can be read against each other: the mechanism (divergence) responds cleanly and monotonically to μ; the outcome (worst-client accuracy recovery) does not. (This figure shares `_plot_vs_alpha`'s legend code with Figures 2 and 3, so its "natural" reference lines were corrected to α∈[1.0,1.7] by the same fix, not left stale.)
...
**Figure 6.** `results/figs/alpha_calibration_fine.png` — TV distance and JS divergence vs. Dirichlet α (log scale), client-weighted mean and cross-client maximum, natural partition's value (dashed) and equivalent-α estimate with 95% CI (dotted line, shaded band).
...
**Figure 7.** `results/figs/client_count_composition_share.png` — shared-test-implied composition share, LR and MLP, K=4 vs. K=130, at α=1.0 (left panel) and α=0.5 (right panel), with 95% CI error bars. **The left panel's K=4 bars (LR ≈77%, MLP ≈75%) are shown for visual completeness but fall below this paper's 2pp reporting floor** (§III-C) — the prose above reports them in absolute terms rather than as the percentages plotted here; the right panel (α=0.5) is the comparison both estimators clear the floor on, and is what the confirmed-prediction claim is built on.
...
**No natural-partition arm is possible for the second dataset.** Diabetes 130-US Hospitals is described by its source publication as spanning "130 US hospitals and integrated delivery networks," but the public release contains no per-record hospital or facility identifier — confirmed by inspecting every column, not assumed from documentation (§IV.A). "130 hospitals" describes the source data warehouse, not an available partition key. All dataset-2 client structure is therefore necessarily synthetic (Dirichlet), unlike the first archive's four real sites — the second dataset cannot corroborate or challenge §V-D's natural-partition calibration on its own institutional structure at all.

**Dataset 2's classical models required class-weighted training to produce a non-degenerate result.** Under the first archive's original unweighted training objective, both LR and MLP converge to a constant majority-class classifier at dataset 2's 8.8% positive prevalence (verified to 2,000 epochs, not an artifact of undertraining). Balanced accuracy alone does not fix this: it corrects the evaluation-metric degeneracy only where an evaluation slice contains both classes, and single-class slices (common at the extreme skew this paper's headline results depend on) still fall back to plain accuracy. Reported dataset-2 results therefore use inverse-frequency class weighting during training in addition to balanced accuracy at evaluation; the first archive's headline numbers, for comparability, are reported both under this same weighting scheme and under the original unweighted protocol (§IV.A).

**The α ∈ [1.0, 1.7] natural-partition calibration (§V-D) does not reconcile with this project's own earlier informal comparison.** `docs/decisions.md` D-037 independently estimated the natural partition as "~α 0.5–1.0" by matching downstream metrics (worst-client accuracy, parameter-divergence magnitude) rather than label-distribution distance — a genuinely different quantity from what §V-D calibrates. Both figures are correct for what they measure; they have not been reconciled into one statement of "how heterogeneous is the natural partition," and the two legacy accuracy-vs-α figures (Figures 2 and 3) still carry the D-037 figure in their legends rather than §V-D's revised range (see the note under Figure 3).
...
[18] S. Liu, "FedGA: A fair federated learning framework based on the Gini coefficient," arXiv:2507.12983, Jul. 2025.

[19] K. Naseer and U. Shoaib, "When more parameters hurt: Foundation model priors amplify worst-client disparity under extreme federated heterogeneity," arXiv:2605.08992, 2026.

[20] S. Rajendran, Z. Xu, W. Pan, A. Ghosh, and F. Wang, "Data heterogeneity in federated learning with electronic health records: Case studies of risk prediction for acute kidney injury and sepsis diseases in critical care," *PLOS Digital Health*, vol. 2, no. 3, e0000117, 2023, doi: 10.1371/journal.pdig.0000117.
...
**Flag 9 — RESOLVED. Both discrepancies from the P-023 reconciliation pass were tracked down rather than left open.**

**(1) VQC timing figure standardized to 13,300× throughout the paper.** `docs/arm4_report.md`, `docs/decisions.md`, and `docs/labbook.md` all independently record the originally-measured figure as ~13,318×; `paper/paper_draft_v2.md` §V-C and (until this pass) `paper/02_related_work.md` used 13,300×. Per instruction, standardized on **13,300×** as the paper's figure — the extra digit in "13,318×" implies a precision the underlying wall-clock measurement does not support. `paper/02_related_work.md` updated to match; the `docs/` historical logs (`arm4_report.md`, `decisions.md`, `labbook.md`) were deliberately **not** rewritten — they are this project's append-only measured record, and 13,318× is what was actually measured and logged at the time; the paper's rounding is a presentation choice made here, not a claim that the log entries were wrong.

**(2) The two uncitable comparator studies were traced (WebSearch/WebFetch), one verified and kept, one dropped.**
- The federated-EHR study across seven real hospitals (AKI/sepsis risk prediction) is **verified with high confidence**: S. Rajendran, Z. Xu, W. Pan, A. Ghosh, F. Wang, "Data heterogeneity in federated learning with electronic health records: Case studies of risk prediction for acute kidney injury and sepsis diseases in critical care," *PLOS Digital Health*, 2(3):e0000117, 2023 — confirmed 7 real hospital site IDs, real-institutional-splits-only, no synthetic Dirichlet comparison, matching the original description exactly. Added as [20], cited in §II.F.
- The "federated learning across real educational institutions" study was **not verified to the required standard and has been dropped, not cited.** A strong title match was found (a ScienceDirect paper on federated academic-risk modeling using parameterized non-IID partitions across educational institutions), but the page returned HTTP 403 and no independent source surfaced the authors, exact year, or DOI within reasonable search effort. Per instruction, not carried into the reference list — this was a comparator, not a load-bearing citation, and an unresolvable reference costs more than it's worth.

**(3) FedGA [18] and Naseer & Shoaib [19] — the two citations newly merged into §II from `02_related_work.md` — were independently verified before being trusted, not carried over on that file's word alone.** Both confirmed directly against their arXiv abstracts: FedGA's title, author (ShanBin Liu), submission date (17 Jul 2025), and Gini-coefficient method all match; Naseer & Shoaib's title, both authors, submission date (9 May 2026), and the exact reported figures (50.1% vs. 32.2% worst-client gap) all match verbatim.
